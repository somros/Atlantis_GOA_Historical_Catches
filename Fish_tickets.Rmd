...
ggplot()+
  geom_sf(data = coast_sf)+
  geom_sf(data = ports, size = 2, color = 'magenta')+
  ...
  labs(title = 'AK ports with highest mean annual (1991-2020) landings according to Fish Ticket data',
       x = '', y = '')
...
nmfs_to_port %>%
  ...
  geom_alluvium(aes(fill = factor(Reporting.Area.Code)), width = 1/12)+
  geom_stratum(width = 1/12, fill = "white", color = "grey")+
  geom_text(stat = "stratum", aes(label = after_stat(stratum)), size = 3)+
  theme_bw()+
  labs(title = '', fill = 'NMFS Area')
